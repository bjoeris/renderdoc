...
echo.
echo    %BOLD%Downloads an "package_name".rdc file from GCS and builds the codegen project for GGP in %RESET%%GREEN%out_dir%RESET%
...
pushd "%src_dir%" > nul
bash -c "source ~/.bashrc && ./build_ggp.sh %config%"
popd
if %errorlevel% neq 0 exit /b %errorlevel%

echo %GREEN%Gathering package data...%RESET%
copy /y %ggp_build_dir%\sample_cpp_trace_elf%suffix% %package_src_dir%\main_ggp > nul
...
ggp ssh shell "tar -xzf /mnt/developer/%package_name%.tgz -C /mnt/developer"

echo %GREEN%Running package...%RESET%
ggp run --cmd=main_ggp --application=dedd6c2b56af4830a4ee5c53b0598264aps1
