...
std::map<VkFramebuffer, std::vector<VkImageView>> framebufferAttachementsMap;
std::map<VkImageView, VkImage> imageViewMap;
std::map<VkImage, VkImageCreateInfo> imageCreateInfoMap;
std::map<VkRenderPass, RenderPassInfo> renderpassInfoMap;
std::map<VkCommandBuffer, RenderPassInfo> commandBufferRenderPassInfoMap;
                            ...
                            const VkAllocationCallbacks *pAllocator, VkImage *pImage)
{
  ...
  imageCreateInfoMap[*pImage] = *pCreateInfo;
  return r;
}
                                ...
                                const VkAllocationCallbacks *pAllocator, VkImageView *pView)
{
  ...
  imageViewMap[*pView] = pCreateInfo->image;
  return r;
}
                                  ...
                                  VkFramebuffer *pFramebuffer)
{
  ...
  framebufferAttachementsMap[*pFramebuffer] = attachments;
  return r;
}
                                 ...
                                 const VkAllocationCallbacks *pAllocator, VkRenderPass *pRenderPass)
{
  ...
  rpInfo.renderPass = *pRenderPass;
  rpInfo.finalLayouts.resize(pCreateInfo->attachmentCount);
  for(uint32_t i = 0; i < pCreateInfo->attachmentCount; i++)
  {
    rpInfo.finalLayouts[i] = pCreateInfo->pAttachments[i].finalLayout;
  }
  assert(renderpassInfoMap.find(*pRenderPass) == renderpassInfoMap.end());
  renderpassInfoMap[*pRenderPass] = rpInfo;
  return r;
}
                               ...
                               VkSubpassContents contents)
{
  ...
  assert(renderpassInfoMap.find(pRenderPassBegin->renderPass) != renderpassInfoMap.end());
  RenderPassInfo rpInfo = renderpassInfoMap[pRenderPassBegin->renderPass];
  for(auto iter = framebufferAttachementsMap[pRenderPassBegin->framebuffer].begin();
      iter != framebufferAttachementsMap[pRenderPassBegin->framebuffer].end(); iter++)
  {
    rpInfo.attachments.push_back(*iter);
    rpInfo.images.push_back(imageViewMap[*iter]);
    rpInfo.imageCIs.push_back(imageCreateInfoMap[imageViewMap[*iter]]);
  }
  commandBufferRenderPassInfoMap[commandBuffer] = rpInfo;
  return;
}
...
void shim_vkCmdEndRenderPass(VkCommandBuffer commandBuffer)
{
  ...
  fn(commandBuffer);
  assert(commandBufferRenderPassInfoMap.find(commandBuffer) != commandBufferRenderPassInfoMap.end());
  copyFramebufferAttachments(&commandBufferRenderPassInfoMap[commandBuffer]);
  return;
}
